// czerwonoczarne.cpp: Określa punkt wejścia dla aplikacji konsoli.
//
...
int main()
{
	...
	int const wielkosc = 10;
	int Tk[wielkosc];          // Tablica kluczy węzłów
	int i, x, i1, i2;
	...
	int wej;
	for (i = 0; i < wielkosc; i++) // Tablicę wypełniamy wartościami kluczy
	{
		Tk[i] = i + 1;
		//cin >> wej;
		//Tk[i] = wej;
	...
	{
		i1 = rand() % wielkosc;     // Losujemy 2 indeksy
		i2 = rand() % wielkosc;
	...
	for (i = 0; i < wielkosc; i++) // Na podstawie tablicy tworzymy drzewo czerwono-czarne
	{
		plik << Tk[i] << " ";
		RBT->insertRBT(Tk[i]);
	}
	...
	{
		i1 = rand() % wielkosc; i2 = rand() % wielkosc;
		x = Tk[i1]; Tk[i1] = Tk[i2]; Tk[i2] = x;
	}

	for (i = 0; i < wielkosc >> 1; i++) // Usuwamy połowę węzłów
	{
		plik << Tk[i] << " ";
